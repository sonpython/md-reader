...
export const Header = () => {
  const { sidebarOpen, previewOpen, editorOpen, theme, autoSaveEnabled, currentFolder, tabs, toggleSidebar, togglePreview, toggleEditor, toggleTheme, toggleAutoSave, setFileTree, reloadTabContent } = useEditorStore()
    ...
    <header className="h-10 bg-editor-sidebar border-b border-editor-border flex items-center justify-between px-4 pl-20 select-none app-drag">
      ...
      <div className="flex items-center gap-2 app-no-drag">
        ...
        <button
          ...
          disabled={previewOpen && !editorOpen}
          ...
          title={previewOpen && !editorOpen ? 'Cannot hide preview when editor is off' : 'Toggle Preview (Cmd+Shift+P)'}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          disabled={editorOpen && !previewOpen}
          aria-label="Toggle Editor"
          className={clsx(
            'p-1.5 rounded transition-colors',
            editorOpen && 'bg-editor-active',
            editorOpen && !previewOpen
              ? 'opacity-40 cursor-not-allowed'
              : 'hover:bg-editor-active'
          )}
          title={editorOpen && !previewOpen ? 'Cannot hide editor when preview is off' : 'Toggle Editor'}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
